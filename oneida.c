 ...
 *
 * Fixes vs v4.0:
 *   [F1] Ch3 flag-entropy direction corrected (high=suspicious)
 *   [F2] Stale channel scores reset at top of compute_scores
 *   [F3] Retrans check moved BEFORE slot write (no stale read)
 *   [F4] Retrans O(n) scan replaced with open-addressed hash set
 *   [F5] usleep(10) busy-wait replaced with sem_post/sem_wait
 *   [F6] calloc-per-flow replaced with pre-allocated slab pool
 *   [F7] pcap_create path with 64 MB kernel ring buffer
 *   [F8] Per-flow alert cooldown (default 5 s, -a flag)
 *   [F9] Nanosecond timestamp precision via PCAP_TSTAMP_PRECISION_NANO
 *   [F10] IPv4 fragment detection + suspicious flag
 * ============================================================ */
...
/* ── Hash table with per-bucket RW locks ── */
typedef struct {
    flow_t          *head;
...
static double window_variance(const void *arr, int n, int is_u32, double *mean_out)
{
                           ...
                           int caplen, const parsed_pkt_t *p)
{
    ...
    /* Ch 1: packet length variability */
    {
        double mean = 0.0, var = 0.0;
    ...
    /* Ch 3: TCP flag entropy — [F1] high entropy = suspicious */
    {
    ...
    /* Ch 4-6: TCP seq/ack/window variance */
    if (flow->l4_proto == IPPROTO_TCP) {
        double dm;
        ft->seq_var    = window_variance(flow->seqs,      n, 1, &dm);
        ft->ack_var    = window_variance(flow->acks,      n, 1, &dm);
        ft->window_var = window_variance(flow->win_sizes, n, 0, &dm);
        int sn = 0, an = 0, wn = 0;
        for (int i = 0; i < n; i++) {
            if (flow->seqs[i])      sn++;
            if (flow->acks[i])      an++;
    ...
    /* Ch 11: retransmission rate */
    {
        ...
        flow->channel_scores[11] = (rr > 0.15) ? 0.50 :
                                   (rr > 0.05) ? 0.25 : 0.05;
    }

    /* Weighted composite */
    static const double W[NUM_CHANNELS] =
        { 2.0,2.0,0.5,1.5,1.0,1.0,1.0,1.0,2.0,1.5,2.0,1.0 };
...
static void process_one(const ring_pkt_t *rp)
{
    ...
    /* [F3] Retrans check BEFORE writing slot */
...
/* ── [F5] Worker thread — semaphore driven ── */
...
static void print_usage(const char *prog)
{
    fprintf(stderr,
        ...
        "  -n             Use nanosecond timestamps\n"
        "  -W <core>      Pin worker thread to CPU core\n"
        "  -C <core>      Pin cleanup thread to CPU core\n"
        "  -v             Verbose\n"
        "  -h             Help\n",
        prog, DEFAULT_THRESH, DEFAULT_COOLDOWN,
        DEFAULT_IDLE, DEFAULT_MAXLIFE);
}
